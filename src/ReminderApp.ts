...
export default class ReminderApp {
  ...
  private handleToggleCompletion(): void {
    ...
    if (this._remindersHandler.size() === 0) {
      ...
      const index = Number(this.getUserChoice("Pick an index", true))
  ...
  private handleModifyReminders(): void {
    ...
    if (this._remindersHandler.size() === 0) {
      ...
      Logger.logReminders(this._remindersHandler.reminders)
      const index = Number(this.getUserChoice("Pick an index", true))
      const desc = this.getUserChoice("Add new description", false)
      this._remindersHandler.modifyReminder(index, desc)
      Logger.log('\n  🏁   Reminder Modified');
  ...
  private handleAddReminder(): void {
    /*
            ...
            ( 4 ) Logger.log('\n  🏁  Reminder Added');
        */
    const description = this.getUserChoice("Enter a description", false)
    const tag = this.getUserChoice("Enter a tag", false)
  ...
  private handleSearchReminders(): void {
    ...
    */
    if (this._remindersHandler.size() === 0) {
      ...
      const keyword = this.getUserChoice("Enter a keyword", false)
